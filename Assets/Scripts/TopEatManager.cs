...
public class TopEatManager : MonoBehaviour
{
    [Header("Panel để hiển thị các quân bài ăn")]
    public Transform topEatPanel;
    ...
    void Start()
    {
        Debug.Log("TopEatManager.Start() chạy");
        ...
        if (cardContainerPrefab != null)
        {
            ...
            containerInstance.SetActive(false); // ẩn để không hiển thị
            ...
            // KHÔNG Destroy(containerInstance)
        }
    }


    public void AddCardToTop(string cardName)
    {
        Debug.Log($"AddCardToTop được gọi với: {cardName}");
        ...
        GameObject card = Instantiate(prefab, topEatPanel);
        card.transform.localScale = Vector3.one * 0.4f;
        cardCount++;
    }

    public void ResetTopEat()
    {
        Debug.Log("ResetTopEat() được gọi");
        foreach (Transform child in topEatPanel)
        {
    ...
    public void AddRandomCardsToTop()
    {
        Debug.Log("AddRandomCardsToTop() được gọi");
