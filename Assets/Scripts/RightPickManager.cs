...
public class RightPickManager : MonoBehaviour
{
    [Header("Panel để hiển thị các quân bài bốc ở bên phải")]
    public Transform rightPickPanel;
    ...
    void Start()
    {
        Debug.Log("RightPickManager.Start() chạy");
    ...
    public void AddCardToRightPick(string cardName)
    {
        Debug.Log($"AddCardToRightPick được gọi với: {cardName}");
        ...
        GameObject card = Instantiate(prefab, rightPickPanel);
        card.transform.localScale = Vector3.one * 0.4f;
        cardCount++;
    }

    public void ResetRightPick()
    {
        Debug.Log("ResetRightPick() được gọi");
        foreach (Transform child in rightPickPanel)
        {
    ...
    public void AddRandomCardsToRightPick()
    {
        Debug.Log("AddRandomCardsToRightPick() được gọi");
