...
public class LeftEatManager : MonoBehaviour
{
    [Header("Panel để hiển thị các quân bài ăn ở bên trái")]
    public Transform leftEatPanel;
    ...
    void Start()
    {
        Debug.Log("LeftEatManager.Start() chạy");
    ...
    public void AddCardToLeft(string cardName)
    {
        Debug.Log($"AddCardToLeft được gọi với: {cardName}");
        ...
        GameObject card = Instantiate(prefab, leftEatPanel);
        card.transform.localScale = Vector3.one * 0.4f;
        cardCount++;
    }

    public void ResetLeftEat()
    {
        Debug.Log("ResetLeftEat() được gọi");
        foreach (Transform child in leftEatPanel)
        {
    ...
    public void AddRandomCardsToLeft()
    {
        Debug.Log("AddRandomCardsToLeft() được gọi");
